...
const MesCollaborations = () => {
  ...
  const applyTransactionUpdate = async (updatedTx, successMessage) => {
    const normalized = transactionToCollaboration(updatedTx, 'fournisseur');
  ...
  const handleMarkWorkDone = async () => {
    if (!selectedCollaboration?.id) return;
    try {
      const updated = await transactionsService.fournisseurWorkDone(selectedCollaboration.id);
      await applyTransactionUpdate(updated, 'Travail déclaré comme effectué.');
    } catch (error) {
      setToast({ message: 'Impossible de déclarer le travail.', type: 'error' });
    }
  };
    ...
    <div className="mx-auto max-w-7xl space-y-6 pb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="relative">
              ...
              />
              <FiFilter className="absolute left-3 top-3 h-5 w-5 text-slate-400" />
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Montant total</p>
              <p className="text-xl font-bold text-orange-700">{formatMoneyFcfa(stats.revenuTotal)}</p>
            </div>
                  ...
                  <tr key={collaboration.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end space-x-2">
                        ...
                        <Link
                          to={`/fournisseur/collaborations/${collaboration.id}/workspace`}
            ...
            <div className="p-6">
              ...
              <div className="space-y-6">
                ...
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                  <h3 className="text-sm font-semibold text-slate-900">Suivi de clôture</h3>
                  <div className="mt-2">
                    {(() => {
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <button
                      ...
                      disabled={selectedCollaboration.raw?.travail_fournisseur_termine}
