...
import { FiFilter, FiCalendar, FiDollarSign, FiUser, FiBriefcase, FiEye, FiDownload, FiClock, FiCheckCircle, FiXCircle, FiAlertCircle } from 'react-icons/fi';
...
const MesTransactions = () => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        setLoading(true);
        console.log('MesTransactions (prestataire) - user:', user);
        const data = await transactionsService.getMyTransactions();
        console.log('MesTransactions (prestataire) - transactions data:', data);
  ...
  const filteredTransactions = transactions.filter(transaction => {
    const matchesSearch = 
      ...
      transaction.demande?.intitule?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const getStatusIcon = (statut) => {
    switch (statut) {
      ...
      case 'acceptee': return FiCheckCircle;
      case 'en_cours': return FiClock;
      case 'terminee': return FiCheckCircle;
      case 'annulee': return FiXCircle;
  ...
  const handleStatusUpdate = async (transactionId, newStatus) => {
    try {
      await transactionsService.updateTransactionStatus(transactionId, newStatus);
      setTransactions(prev => prev.map(t => 
        t.id === transactionId ? { ...t, statut: newStatus } : t
      ));
      setToast({
  ...
  const stats = {
    ...
    enCours: transactions.filter(t => t.statut === 'en_cours').length,
    terminees: transactions.filter(t => t.statut === 'terminee').length,
    montantTotal: transactions
      ...
      <div className="bg-white shadow rounded-lg p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Historique de vos transactions en tant que prestataire</p>
          </div>
                  ...
                  <tr key={transaction.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm">
                        ...
                        <div className="text-gray-500">
                          {transaction.demande?.lieu_intervention}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Link
                        to={`/prestataire/transactions/${transaction.id}`}
