...
import { FiMessageSquare, FiSend, FiSearch, FiUser, FiPhone, FiMail, FiBriefcase } from 'react-icons/fi';
...
const MesMessages = () => {
  ...
  useEffect(() => {
    const fetchConversations = async () => {
      try {
        setLoading(true);
        console.log('MesMessages (fournisseur) - user:', user);
        const data = await messagesService.getMyMessages();
        console.log('MesMessages (fournisseur) - conversations:', data);
  ...
  const getUserIcon = (user) => {
    if (user?.type_utilisateur === 'fournisseur') return FiBriefcase;
    if (user?.type_utilisateur === 'client') return FiUser;
    return FiUser;
  };
